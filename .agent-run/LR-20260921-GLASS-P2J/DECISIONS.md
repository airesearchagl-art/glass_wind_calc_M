...
> ```
>
> **【D-042による再訂正（検証9 F9-02）】上の 1104 / 2412 も再現しない。**
> グリッドを**サイズだけ**で書いていたのが原因で、検算できなかった。
> 定義を明記して再実測する:
>
> ```text
> グリッド: 3幹（図面 / 構造計算書 / plan）
>         × 46 具体拡張子 × dot 2形（. ．） × 拡張子の半角/全角 = 552 base
>         + 552 × 末尾全角英数字 8 形（ＡＺａｚ０９１５） = 計 4968 証人
> 対象 tree: df88109（D-040 の表が記述している tree）
...
> **【D-042による訂正（検証9 F9-03）】全削除は行き過ぎだった。**
> lookahead を完全に外すと `.doc` が `document` の中でマッチし、
> **この repository に実在する識別子**が落ちていた:
>
> ```text
> window.document を直接触らない      拒否されていた
> e.target.value を読む              拒否されていた
> Workspace.csvEscape を使う         拒否されていた（workspace.js の実在の export）
> config.documentation を参照        拒否されていた
> ```
>
> **【D-043による訂正（検証10 F10-07）】「この repository に実在する識別子」は誤り。**
> 実際に数えた結果（test を除く）:
>
> ```text
> window.document       1 件 —— しかも evidence.js 自身の、この規則を正当化するコメントの中
> e.target.value        0 件
> config.documentation  0 件
> node.documentElement  0 件 / obj.docs 0 件 / gzip ・ tarball 0 件
> csvEscape             4 件 —— これだけが実在の export
> ```
>
> つまり主張の根拠が**自分のコメントを引用していた**。循環している。
> 実際の publicDescription 9 件は lookahead の有無によらずどちらも影響を受けない。
> 語境界を残す判断自体は変えていないが、その根拠は弱かった。
>
> その上このコストはどの artifact にも記録していなかった——「純粋な利得」として書いていた。
> 正しい幅は `(?![A-Za-z])`（英字のみ）で、これなら
> `構造計算書.pdf2` も `図面．ｄｗｇ１` も拒否しつつ、上の 4 件は通る。
> P2J-S35 が両方向を固定する。
...
- union なので単調——畳みは拒否を増やすだけ。F1 の形の回帰は**原理的に起きない**。
  ...
  > **【D-042による訂正（検証9 F9-01）】この文は誤り。**
  > `A ∨ B ⊇ A` は成立するが、単調なのは **raw に対してだけ**である。
  > 同じ commit で fold 関数自体を広いものへ**差し替えていた**ので、
  > 実際の比較は `旧: raw ∨ narrowFold` 対 `新: raw ∨ wideFold` で、
  > どちらも他方を含まない。結果 F1 クラスが再び開いた
  > （`構造計算書（最新）．ｐｄｆ` が df88109 で拒否 → 4573d01 で受理、1472形）。
  > D-042 で normalizer を**集合**にした。追加は単調、差し替えと削除は単調ではない。
...
## D-042 — 正規化は集合であり、差し替えは単調ではない（9回目検証）

独立検証9 の判定は **FAIL（P0）**。D-041 が「原理的に起きない」と宣言した
F1 クラスが、別の符号化で再び開いていた。

### 何が起きていたか

```text
                          df88109  4573d01
構造計算書（最新）．ｐｄｆ      拒否     受理   ← 1472形が反転
図面（改訂）．ｄｗｇ          拒否     受理
構造計算書（最新）.pdf         拒否     拒否   ← raw が拾うので無事
```

### なぜ「単調だから安全」が偽だったか

`A ∨ B ⊇ A` は正しい。しかしそれが保証するのは
「**raw 単体より弱くならない**」だけである。
D-041 は同じ commit で fold 関数を**広いものへ差し替えていた**ので、
実際の比較はこうだった:

```text
旧: raw ∨ narrowFold
新: raw ∨ wideFold
どちらも他方を含まない。単調性はこの比較について何も言わない。
```

`構造計算書（最新）．ｐｄｆ` を拒否するには
「`．`→`.` にするが `（）` は触らない」畳みが要る。広い畳み 1 つではこの 2 つを同時に満たせない。

さらに悪いことに、D-041 は**この欠陥を正確に説明していたコメントを削除した**——
「範囲を狭める必要がなくなり、問題自体が消えた」という理由で。消えていなかった。

### 採用した形

```js
var TEXT_NORMALIZERS = [foldFullwidthFilenameChars, foldFullwidthAscii];
// 規則は raw と各正規化形のいずれかでマッチすれば拒否する。
```

- normalizer の**追加**は単調。**削除と差し替えは単調ではない**。
  これを文章ではなく P2J-S34 で固定した（各 normalizer が単独で担当する形を 1 つずつ置く）。
- `hasFullwidthForm` は削除した。範囲を 2 重に持っており、そちらの下端だけが
  未固定で変異が生き残っていた（F9-04）。正規化形を作って元と比べるだけにした。

### 語境界は raw 側だけに戻した（F9-03）

D-041 の「lookahead 全削除」は行き過ぎで、`window.document` や
`Workspace.csvEscape`（workspace.js の実在の export）が落ちていた。
`(?![A-Za-z])`（英字のみ）に戻すと両方成立する。

ただし正規化後の形ではこの前提が崩れる——`構造計算書．ｐｄｆＡ` を畳むと
`...pdfA` になるが、この `A` は英単語の続きではない。
よって `normalizedPattern`（語境界無し）を正規化形専用に置いた。
正規化形が存在するのは入力に全角が含まれていた場合だけなので、
そこで英単語の続きを守る必要はない。

### test 側の欠陥が本体だった

S24 には「装飾付き幹」も「全角拡張子」もあったが、
前者は半角 dot・半角拡張子（raw が拾う）、後者は装飾無し。
**交わる形をどちらも含んでいなかった**。行と列を個別に見ている限り
グリッドの穴は見えない——検証8 が S29 で指摘したのと同じ誤りが、
同じファイルの 1 つ上の test で繰り返されていた。

P2J-S33 は直積を**生成**する（3幹 × 46拡張子 × dot 2形 × 半角/全角 × 装飾 8形
= 4416 組）。装飾無し base が拒否されていることを positive control にしている。

### mutation

```text
R9-01 normalizer を wide だけへ（F9-01 を復元）  KILLED (S33, S34)
R9-02 normalizer を narrow だけへ              KILLED (S34, S32)
R9-03 normalizedPattern を使わない            KILLED (S35, S29)
R9-04 raw の語境界を外す（F9-03 を復元）      KILLED (S35)
R9-05 頭文字クラスを @ まで広げる            KILLED (S27)
R9-06 拡張子 doc[xm]? → doc                 KILLED (S24, TB19)
R9-07 拡張子 tiff? → tif                     KILLED (S24, TB19)
R9-08 拡張子 ppt[xm]? → ppt                 KILLED (S24, TB19)
R9-09 caseId 規則の i フラグ                KILLED (TB19)
```

9/9。R9-06..08 は検証9 F9-05（corpus が検査対象の定数から生えていた）への対応で、
TB19 に**定数とは独立な**具体拡張子 46 件の期待列を置いた。
...
## D-043 — 幹を捨てた（10回目検証 / 局所修理をやめて設計を変えた）

独立検証10 の判定は **FAIL**。そして「修理は収束していない」という
構造的な診断がついた。検証した結果、その診断は正しい。

### 決定的な指摘（F10-02）

```text
plan (1).pdf          受理   ← Explorer が重複ファイルに自分で付ける名前
構造計算書 (2).pdf     受理
構造計算書(最新).pdf    受理   ← F1 と同じ形の ASCII 版
構造計算書 .pdf        受理
10304 証人中 10304 素通り（100%）。対して装飾無し 736/736 は拒否。
```

F1 は「畳みが `（）` を区切り文字に変えて幹を切る」だった。
**ASCII の `(` `)` は最初から区切り文字だった**ので、同じ形がこの規則が
書かれた符号化そのものですっと開いていた。6 度の修理は全角軸だけを見ていた。

### なぜ局所修理では終わらないのか

述語は「区切り以外の最大連結 + `.ext`」だった。この形は要求が矛盾する:

```text
幹が散文を飲み込まないよう   → 区切り文字は多くしたい
区切りを含むファイル名を見逃さない → 区切り文字は少なくしたい
space / 括弧 / 引用符 / カンマは**両方**である（Windows・macOS で合法なファイル名文字）。
```

どちらを取ってももう一方が壊れるので、次の corpus が押さえなかった区切りで
必ず新しい F10-02 が出る。**テストを増やす問題ではなかった。**

### 採用した形: 幹を要求しない

```js
'\\.(' + PRIVATE_DOCUMENT_EXTENSION_SOURCE + ')(?![A-Za-z])'
```

これは tag 規則で既に下したのと**同じ判断**である——
「巧妙な例外を足さない。偽陽性を受け入れて素通りを消す」。
この理屈をファイル名規則に適用していなかったのが見落としである。

```text
同時に閉じた: F10-01（正規化形への境界無し適用が散文を巻き込んでいた）
              F10-02（区切り文字による 100% 素通り）
              F10-03（パターン差し替えによる非単調）
コスト   : 文中の裸の `.zip` も落ちる。出荷済み 9 件は影響 0（実測）。
副次効果 : `{1,120}` の ReDoS 面と QD-J04 の未固定コストガードが消滅。
```

### normalizer を測り直した

幹を捨てた結果 `foldFullwidthFilenameChars`（狭い畳み）は**寄与 0**になった——
あれは幹を守るためだけのものだった。全規則を含む corpus で行動差 0 を
測った上で削除した（推論ではなく測定で）。

代わりに **dot 写像**を追加した（F10-06）。`。`(U+3002) は日本語入力モードの
ピリオドキーが出す文字で、F1 が対象にした `．` よりありふれているのに
6 度の修理を通して一度も見ていなかった。NFKC 不変なので NFKC では閉じない。
中黒（`・`）は意図的に除外——`PDF・doc形式で提出` のような普通の技術文を落とすため。

正規化形は**閉包**を取る。`構造計算書。ｐｄｆ` は dot 写像だけでも
全角畳みだけでも届かず、合成で初めて届く。

### test を corpus から不変式へ

P2J-S33 を書き直した: 「**dot 形 + 拡張子を含み、直後が ASCII 英字でなければ、
前が何であれ拒否**」。幹を捨てたのでこれが言えるようになった。
前置きに旧幹の区切り文字を全部入れ、拡張子は大文字・全角大文字も回す
（旧版は `toFull(ext)` を小文字定数から作り、全角大文字を一度も生成していなかった）。

### mutation

```text
R10-01 幹の要求を復元            KILLED (S24, S33)
R10-02 dot 写像を削除             KILLED (S33, S34)
R10-03 wide fold を削除           KILLED (S24, S33)
R10-04 閉包をやめ 1 回適用          KILLED (S33, S34)
R10-05 wide から全角大文字を除く    KILLED (S24, S33)
R10-06 control-character 上端      KILLED (S36)
R10-07 語境界を外す              KILLED (S35)
R10-08 中黒を dot 写像へ（過剰拒否）  KILLED (S34)
R10-09 control-character 下端      KILLED (S01)
```
...
過去の全 head に対して regression 0。唯一の差分は 4573d01 の
...
R11-02 closure guard を黙って打ち切る   【訂正】SURVIVED（下記）
drop U+3002 / FF61 / FE12 / 2024 / FE52 / 2027 /
     2E33 / 0387 / 06D4 / 0701 / A4F8 / 02D9   すべて KILLED (S37)
```

【D-045による訂正（検証12 F12-02）】**14/14 は 13/14 である**。
R11-02 の忠実な mutant（上限は残し `throw` を `break` へ）は生存する。
normalizer が冪等かつ可換なので閉包は高々 4 回しか回らず、
`guard > 16` の分岐は**到達不能な死んだコード**だからである。
死んだコードの変異は原理上殺せない。
...
## D-046 — 手書きの列挙を Unicode クラスへ / 自分で DECISIONS.md を壊した（13回目検証）

独立検証13 の判定は **FAIL**。

### F13-02 — まずこれを記録する: DECISIONS.md を 45 決定分削除していた

前回の commit で `DECISIONS.md` が 1803 行 / 45 決定 → 87 行 / 1 決定 になっていた。
D-000（digest 定義）も D-037（この guard を持つべきかの Human Gate 申し送り）も消えていた。

原因は特定できている:

```python
io.open(path, 'w', encoding='utf-8').write(text)
#   'w' は open した瞬間に truncate する。
#   write が UnicodeEncodeError（astral 文字の surrogate）で落ちると、
#   ファイルは**空のまま残る**。次の script がその空ファイルに追記した。
```

復旧し、D-044 の訂正と D-045 を再適用した（46 決定 / 1891 行）。
以後 artifact の書き込みは **encode を先にやってから temp → rename** する。
（digest は RUN_STATE / RUN_MANIFEST にもあるので Resume は壊れていなかった。）

### F13-01 — 「種類を追加した」だけでは 5% しか閉じていなかった

D-045 の看板は「削除写像という**種類**を追加した」だった。種類は合っていたが、
中身が**手書き 24 文字の列挙**だった。

```text
実在する不可視文字（Cf + variation selector） : 430
宣言していた                              :  24
www / url-scheme / email-like / filename が 407/430 で素通り
（U+FE0F は絵文字対応エディタが日常的に出す）
```

検証11 の「手リストへのメンバシップ test は射影」が、
**それを直すための fix の中で再発していた**。

対応: `/[\p{Cf}\p{Variation_Selector}\u034f]/gu` へ。導出されたクラスにした。

### F13-03 — dot 集合を 12 → 8 へ縮めていた（回帰）

「基準を一貫させる」という理由で 4 文字を外し、16 形の拒否を失っていた。
縮めるのは回帰だと D-044 に**自分で書いていた**。
検証13 は 7,084 の実散文で測り、戻しても偽陽性 0 だと示した。戻した。

さらに悪いのは、**数値を反証可能にするために commit した tool が
これを clean と報告していた**ことである——`corpus.mjs` の `DOTS` が
12 メンバ中 6 しか生成していなかった。corpus は前回の検証の軸に沿って
拡張され、**今回の diff が変えた軸には沿っていなかった**。
→ `DOTS` を全メンバ + 除外側へ、`INVISIBLE` を拡張、¥ 軸を追加。
   corpus 212,058 → 565,758。

### F13-07 — 日本語 Windows の ¥ path が全部素通り

```text
C:\u00a5Users\u00a5tanaka\u00a5案件      受理 → 拒否
\uffe5\uffe5fileserver\uffe5案件           受理 → 拒否（UNC）
価格は\u00a51,500,000とする            受理のまま（通貨は巻き込まない）
```

`windows-absolute-path` / `unc-path` は本Campaign で一度も見ていなかった規則である。

### F13-04 — dot 集合の基準を 3 度言い直し、三度とも例外が出た

今回は言い直さない。**導出原理は無い**と認め、列挙であると明記して
Human Gate へ送る（QD-J13）。QD-J11 の旧基準の行も訂正した（F13-05）。

### mutation

```text
R13-01 FORMAT_CHARS を手書き 24 へ戻す      KILLED (S39)
R13-03 dot 集合を再び 8 へ                KILLED (S37)
R13-04 ¥ fold を外す                     KILLED (S41)
R13-05 U+2028/2029 を除く                KILLED (S41)
R13-06 url-scheme を https? へ            KILLED (S41)
R13-07 windows path を大文字だけへ         KILLED (S41)
R13-08 opaque-long-token を {29,} へ      KILLED (S41)
R13-09 opaque クラスから - を除く          KILLED (S41)
R13-02 FORMAT_CHARS から /g を外す        SURVIVED —— **等価**
```

R13-02 は検証13 が behaviour-changing としたが、実測では等価である:
閉包が normalizer を不動点まで反復するので、`/g` が無くても 1 文字ずつ削られて
同じ形に到達する（`p<ZWSP>d<ZWSP>f` も両方で拒否）。関数単体では差が出るが、
guard 全体では出ない。
...
## D-050 — \u00a5 の通貨/path 曖昧は機械的に決定しない。fail-closed で切る

```text
状態 : 確定（§17 の Required Fix 範囲）
```

3 round この境界をいじった。毎回「過剰 reject を消す」と「本物の穴を開ける」を
交換していただけだった。

```text
F14-03  fold に (?!\d)     → F15-A1: 数字始まり segment の path が全滑り
F15-A1  規則側に通貨例外  → F16-01: ASCII の C:\\500 が全滑り
```

置き場所を変えても治らない理由:

```text
規則側に置く  → 規則は `\\` も `/` も見るので、\u00a5 にしか無い曖昧さの
                例外が ASCII 形を巻き込む
fold 側に置く  → `C:\u00a52024` と `B:\u00a52024`（2024円）は文字列として同一。
                fold は文脈を持たないので区別できない
```

つまりこれは**実装の不備ではなく決定不能**であり、
Human Gate が 3 規則について認定した open-set 問題が
構造規則の内側に小さく現れたもの。

よって通貨例外を**削除**し、`Price:\u00a5500` が落ちることを QD-J19 として
開示する。過剰 reject は fail-closed 側であり、「この文章を公開してよいか」は
元々 Human Gate の所管。

学んだこと: **過剰 reject を計測していない数字を根拠に規則を緩めない**。
