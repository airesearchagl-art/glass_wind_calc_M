...
- Current wave: Wave 0 — Fresh Gate / Run Artifact / baseline / 可用性判定 完了
...
Wave 1: §11 のとおり既存 Evidence architecture を inventory する。
`project-config/evidence.js` / `evidence-ledger.js` の contract を確認し、
checkedAt validator / public source reference validator / promotion gate /
reconciliation status を**再実装せず再利用する**前提を固める。
あわせて4つのunresolved factのdomain model（floor key / zone key の正）を実測する。
