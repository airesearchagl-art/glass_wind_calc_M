...
## QD-J24 — 最終確認が残した観察 2 件（指摘ではない）

凍結後なので **本 Phase では直さない**。次に test を触るときのために残す。

### (a) FP-01b の `/\.\d+\./` は合成経路専用

```text
現状 : FP-01b が自分で作った合成経路にだけかけているので安全。
危険 : これを実の inventory へ向けると偽陽性になる。
       実の経路には `…positivePressureByFloor.1.evidence.publicDescription` のような
       **正当な数字 key** があり、配列添字と区別できない。
```

### (b) FP-01 の期待側 walker は実装と同じ algorithm

```text
実装と test が同じ歩き方（readdirSync + 再帰歩行）を持つ。
現在対称性を破っているのは FP-01b の**ハードコードされたリテラル経路**だけ。
```

これは QD-J20 / J21 / J23 と同じ家族である:

```text
実装と計器が同じ仮定を共有すると、その軸には誰も気づかない。
```

本Campaign の累積記録としては、**この家族が最も繰り返し発生した欠陥類型**である。

```text
F3      拡張子集合を 2 か所に持ち片方だけ更新した
F9-04   範囲を 2 重に持ち下端だけが未固定だった
F13-03  corpus の DOTS が手書きで重複・欠落
F14-04  同上（別の軸）
F15-E3  path core が全部英字始まりで \u00a5 軸が数字を生成できなかった
F16-03  bare 1-3 桁 segment が無く、通貨例外を REGRESSIONS: 0 と認定した
F16-07  変異演算子が使い捨て script にしか無く score を再現できなかった
QD-J21  probe が message を見ていなかったので「等価」と誤報した
FP-01   lint の検査対象が手書きだった
QD-J23  readdirSync が実装・test 両方で非再帰
```

**次 Phase への提言**: 規則や正規化を増やすより、
「計器が実装と独立であること」を構造的に保証する方が利回りが大きい。
