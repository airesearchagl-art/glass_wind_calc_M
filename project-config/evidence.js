...
(function (global, factory) {
  ...
  var PUBLIC_UNSAFE_TEXT_PATTERNS = [
    ...
    //
    // ── なぜ「属性の形だけ拒否する」ではいけないか（Wave 6 再検証 F1）──
    //
    // Wave 6 の最初の修理は「タグ本体が**属性の形をしている**ものだけ拒否する」
    // という書き方をした。これは意図を**裏返して**しまっていた。
    // 属性の文法に合わない本体は、どれも一致せずに**素通り**するからである:
    //
    //     <img src=x onerror=alert(1)>   拒否（属性の形）
    //     <img src=x onerror=alert`1`>   **素通り**（バッククォートが値クラス外）
    //     <script "q">  <img 1=2>  <img .x=1>  <img x=a'b>   いずれも素通り
    //
    // 崩れたタグほど通る、という逆転が起きる。
    //
    // ── 正しい線引き ────────────────────────────────────────
    ...
    // （W / H / P / Q / Z はこのドメインの変数名そのもの）。
    // 散文の本体には日本語が入り、タグの本体には入らない。そこで:
    //
    //     タグの形をした開き括弧は、**本体に日本語が含まれない限り拒否する**
    //
    //     <H かつ P>Q          本体に「かつ」がある → 散文として通す
    //     <img src=x onerror=…> 本体に日本語が無い   → タグとして拒否
    //     5<Z<40               `>` で閉じていない    → 通す
    //
    // 本体の書式を問わないので、崩れたタグも同じように落ちる。
    // 入れ子の量指定子も無くなるため、backtracking の懸念も構造的に消える。
    //
    // トレードオフは明示しておく: `<T 日本語>` のような、日本語を含む
    // タグ形の文字列は散文として通る。privacy境界としては許容する
    // （日本語を含む時点で、機械的なタグ/属性の運搬体ではない）。
    { name: 'html-like-tag', pattern: /<\/?[A-Za-z][A-Za-z0-9-]*(?:[\s\/][^<>\u3000-\u9FFF\uF900-\uFAFF\uFF00-\uFFEF]*)?>/ },

    // タグ以外のmarkup構文。`-->` は技術散文の矢印（`A --> B`）と衝突するため入れない。
