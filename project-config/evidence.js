...
(function (global, factory) {
  ...
  var PUBLIC_UNSAFE_TEXT_PATTERNS = [
    ...
    //
    // さらに「`<`+英字 … `>`」だけでは足りない（独立検証の指摘 A1）。
    // それだと `W<H かつ P>Q である。` を拒否してしまう。
    // W / H / P / Q / Z は本ドメインの変数名そのものなので、この形の散文は
    // 現実に書かれうる。実際にはfail closed（誤って受理する側ではない）だが、
    // 将来のEvidence散文が理由の分からないまま書けなくなる。
    //
    // そこでタグ本体に「**属性の形をした内容**」だけを許す:
    //   `<name>` / `</name>` / `<name attr>` / `<name attr=value>` /
    //   `<name attr="value">` / `<name/>`
    // `<H かつ P>` は `かつ P` が属性の形になっていないので一致しない。
    { name: 'html-like-tag', pattern: /<\/?[A-Za-z][A-Za-z0-9-]*(?:\s+[A-Za-z_:][A-Za-z0-9_:.-]*(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^\s"'<>`]+))?)*\s*\/?>/ },
